...
function getSkuSettingsForDialog(skuId) {
  ...
  var fields = [
    ...
    'FBM_SELLING_PRICE', 'FBM_FULFILLMENT_COST', 'FBM_CONVERSION_RATE', 'FBM_START_DATE_OVERRIDE',
    'DAILY_VELOCITY',
...
function saveSkuSettingsFromDialog(skuId, values) {
  ...
  for (var key in values) {
    ...
    if (textFields.indexOf(key) > -1) {
      range.setValue(val || '');
      // If FBM SKU ID was just set, flag it so we can link FBM fields after the loop
      if (key === 'FBM_SKU_ID' && val && val !== '') {
        fbmSkuWasSet = true;
      }
      continue;
    }
...
function waitForGorillaFormulas(skuId) {
  ...
  ss.toast(
    'Waiting for Gorilla to compute data — this may take up to 45 seconds…',
    'Gorilla ROI',
    45
  );
